...
for data in materiales.keys():
    ...
    if response.status_code == 200:
        ...
        if data == "transport":
            hoja=hojas[data]
            fila_vacia = encontrar_fila_vacia(hoja)
            ...
            for qly in material_data:
                ...
                if quality not in registros_calidad[data]:
                    ...
                    print(f"{data} Q{quality} registro:{registros_calidad[data][quality]}")
                    new_material_data = process_data(qly)
                    hoja = hojas[data]
                    if quality == 0:
                        fila_vacia = encontrar_fila_vacia(hoja, quality)
                        print(f'hoja {hoja}, fila vacia {fila_vacia}, data {new_material_data}')
                        guardar_datos_calidad_0(hoja, fila_vacia, new_material_data)
                    elif quality == 1:
                        fila_vacia = encontrar_fila_vacia(hoja, quality)
                        print(f'hoja {hoja}, fila vacia {fila_vacia}, data {new_material_data}')
